...
impl<'a> Window<'a> {
    ...
    pub fn new_with(width: u32, height: u32, title: &'a str) -> Window<'a> {
        ...
        Window {
            ...
            m_window_handle: Box::new(RenderWindow::new(VideoMode::new(width as u32, height as u32, desktop_mode.bits_per_pixel), title, Style::CLOSE, &Default::default()))
        }
    }

    pub fn get_handle(&mut self) -> &mut RenderWindow {
        &mut *self.m_window_handle
    }
    ...
    pub fn set_fullscreen(&mut self, fullscreen: bool) {
        ...
        if !self.m_fullscreen && fullscreen {
            self.m_window_handle.close();
            self.m_window_handle = Box::new(RenderWindow::new(VideoMode::new(desktop_mode.width, desktop_mode.height, desktop_mode.bits_per_pixel), self.m_window_title, Style::FULLSCREEN, &Default::default()))
        } else if self.m_fullscreen && !fullscreen {
            self.m_window_handle.close();
            let new_width = (desktop_mode.height as f32) * self.m_initial_aspect_ratio;
            self.m_window_handle = Box::new(RenderWindow::new(VideoMode::new(new_width.floor() as u32, desktop_mode.height, desktop_mode.bits_per_pixel), self.m_window_title, Style::CLOSE, &Default::default()))
        } else {
            // Nothing
        }
    }
